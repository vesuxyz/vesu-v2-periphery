...
function readArtifacts(contract: string): DeclareContractPayload {
  return {
    contract: readArtifact(`./target/release/vesu_multiply_${contract}.contract_class.json`),
    casm: readArtifact(`./target/release/vesu_multiply_${contract}.compiled_contract_class.json`),
  };
}
